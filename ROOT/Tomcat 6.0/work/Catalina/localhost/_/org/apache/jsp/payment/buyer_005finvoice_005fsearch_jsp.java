...
public final class buyer_005finvoice_005fsearch_jsp extends org.apache.jasper.runtime.HttpJspBase
...
sql = " select  " +
      ...
      "       case a.status when 'P' then 'Create Invoice' when 'S' then 'Send Invoice' when 'R' then 'Receive Invoice' when 'C' then 'Paid' end , " +
      "       date_format(rcv_date,'%Y/%m/%d') " +
      "  from invoice_01t a left outer join login_01t b on( a.buyer_id = b.id )  left outer join  login_01t c " +
      "         on ( a.sup_id = c.id)  " +
...
if (iRet > 0 ) {
  for (i=0; i < iRet; i++) {
    ...
    status = matrix.getRowData(i).getData(10);
    rcv_date = matrix.getRowData(i).getData(11);
    ...
    outS += "<tr bgcolor=" + color_code + ">" +
            ...
            "<td align=center > <font size=2>" + rcv_date + "</font></td>" +
...
for (int k = thisYear - 4; k <= thisYear; k++) {
  optionYears += "<option value='" + k + "' " + (ag_year.equals(k + "") ? "selected" : "") +">" + k + "</option>";
      ...
      out.write("  document.frmMain.action = './buyer_invoice_details.jsp';\n");
      ...
      out.write("<FORM NAME=frmMain action=\"buyer_invoice_search.jsp\" METHOD=POST >\n");
 ...
 if(ag_month.equals("12")) out.print("SELECTED"); 
      ...
      out.write("<TD width=15%><FONT size=2>Invoice Number </FONT></TD>\n");
