...
public class _po_0confirm__jsp extends com.caucho.jsp.JavaPage
{
...
try {
  ...
  pstmt = connScm.prepareStatement(sql1);

  for (int i = 0; i < po_no.length; i++) {
    try {
      // insert po_style_etc info
      if (!poList.contains(po_no[i])) {
        pstmt.setString(1, po_no[i]);
      ...
      params.put("lp-po-order-no", po_no[i]);
      ...
      confirmedPOs += (confirmedPOs.length() > 0 ? ", " : "") + po_no[i];
    } catch (Exception ie) {
      try { connScm.rollback(); } catch (Exception ex) {}
      failedPOs += (failedPOs.length() > 0 ? ", " : "") + po_no[i];
      //ie.printStackTrace();
    }
  }
  ...
  System.out.println("Exception in po_confirm : " + e.getMessage());
  throw e;
} finally {
  if (pstmt != null) {
...
out.println(CopyRightLogo());
    ...
    throws javax.servlet.ServletException
  {
    ...
    depend = new com.caucho.vfs.Depend(appDir.lookup("order/po_confirm.jsp"), -7570822773595428510L, false);
    com.caucho.jsp.JavaPage.addDepend(_caucho_depends, depend);
  ...
  static {
    ...
    _jsp_string0 = "\n\n\n\n\n\n".toCharArray();
    _jsp_string5 = "\n\n</td></tr></table>\n<form name='form1' method='post' action='./po_confirm_search.jsp'>\n<input type='hidden' name='ag_brand' value='".toCharArray();
