...
public class _dsk_0po_0confirm_0search__jsp extends com.caucho.jsp.JavaPage
{
...
sql = " select  to_char(c.xpoa_ts_created+10/24, 'dd/mm/yyyy'), "
    ...
    + "           on g.cre_accountcode = j.accountcode "
    + " where   a.backorder_flag = ' ' "
    + " and     j.na_type = 'C' "
    + " and     j.na_country_code in "
    + "         ( "
...
try {
  ...
  System.out.println("Exception in dsk_po_confirm_search : " + e.getMessage());
  throw e;
} finally {
  if (connPronto != null) {
...
for (int i = 0; i < iRet; i++) {
  ...
  for (int k = 0; k < iRet3; k++) {
    mrOptions += "<option value='" + matrix3.getRowData(k).getData(0) + "'";
    
    if (!isFound && mr.equals(matrix3.getRowData(k).getData(0))) {
      mrOptions += " selected";
      isFound = true;
    }
    
    mrOptions += ">" + matrix3.getRowData(k).getData(1) + "</option>";
  }
  ...
  outS += "<tr align='center' bgcolor='" + colour_code + "'>"
        ...
        + " <td style='color:" + colour_font + "'>" + brandName + "</td>"
        + " <td style='color:" + colour_font + "'>" + buyer_name + "</td>"
        + " <td style='color:" + colour_font + "' align='right'>" + (currency_unit_price > 0.0d ? StringUtil.formatNumber(currency_unit_price, MONEY_FORMAT) : "TBA") + "</td>"
        + " <td style='color:" + colour_font + "'><select name='mr'>" + mrOptions + "</option></td>"
...
out.println(CopyRightLogo());
    ...
    throws javax.servlet.ServletException
  {
    ...
    depend = new com.caucho.vfs.Depend(appDir.lookup("order/dsk_po_confirm_search.jsp"), 6921281889587345481L, false);
    com.caucho.jsp.JavaPage.addDepend(_caucho_depends, depend);
