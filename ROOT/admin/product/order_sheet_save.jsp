...
<%
   ...
   try{
    ...
    sql = " INSERT INTO order_sheet"
          + " ( "
          + "   po_num, comment, main_label, size_label, care_label, hanger_tape, price_tag, hanger_tape_simple, packing"          
          + " ) "
          + " Values "
          + " ( ?, ?, ?, ?, ?, ?, ?, ?, ?); ";
...
//    sql = "select id, po_num from order_sheet;";      
//    int iRet = dataProcess.RetrieveData(sql, matrix, conn);
//    for (int i = 0; i < iRet; i++) {
//        int j = 0;  
//        String id = matrix.getRowData(i).getData(j++);
//        String tmp_po_num  = matrix.getRowData(i).getData(j++);
//        outPut += id +" " +tmp_po_num+"<br/>";
//    }
    ////  end select  //////
