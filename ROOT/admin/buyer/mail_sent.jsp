...
String sql = "SELECT seqno, receiver_id, receiver_name, title, " +
			...
			" read_flag " +
		" FROM mail_01t " +
		" WHERE userid = '" + userid + "' AND del_flag is null " +
		" ORDER BY seqno ASC ";
...
if( connState ) {
	...
	while( iRet > 0 ) {
		...
		String title = matrix.getRowData(iRet-1).getData(3);
        if( title.length() > 48 ) {
		...
		outS += "<tr><td></td>\n<td align=center><font size=2>" +
            ...
            "\"></td>\n<td><font size=2>" +
			matrix.getRowData(iRet-1).getData(2) +
			"</td>\n<td><font size=2>" +
			"<a href=\"sent_read.jsp?seqno=" + 
			matrix.getRowData(iRet-1).getData(0) + "\">" +
			title + 
			"</a></td>\n<td><font size=2>" +
			matrix.getRowData(iRet-1).getData(4) +
			"</td>\n<td align=center><font size=2>";
		String read = matrix.getRowData(iRet-1).getData(6);
		if( read.equals("1") ) {
			outS +=  matrix.getRowData(iRet-1).getData(5) + "</td></tr>\n" +
...
<TD><IMG src="img/mail_s_sent_over.jpg" border=0></TD>
...
<TD width=80 background="img/inbox_back.jpg"><IMG SRC="img/sent_to.jpg"></TD>
